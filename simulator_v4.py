# 가상 팀 회의 시뮬레이터 v4 — 의제 분해 + 결정 원장 + 맥락 주입 (로컬 LLM 지원 & 인간적 공감 요소 & 주도성 가중치 및 대인관계 반영)
...
SAVED_PARTICIPANTS = [
    {
        ...
        "initiative": 0.8,
        "quirks": [
            "말머리에 항상 '허허...' 하며 부드럽고 따뜻하게 대화를 시작함", 
            "의견이 대립할 때 '양쪽 다 일리가 있네요'라며 갈등을 풀고 합의안을 찾으려 함"
        ],
        "relations": {
            "박범서": "부정적이고 귀찮아하는 태도를 나무라지 않고 온화하게 다독거리며 참여를 독려함"
        }
    },
    {
        "name": "문경수", 
        "role": "일처리 잘하고 적극적 의사 표명, 레드팀", 
        "preset": "2",
        "initiative": 0.7,
        "quirks": [
            "데이터, 예산, 팩트 중심의 예리하고 깐깐한 질문을 던짐", 
            "막연한 계획보다 숫자로 검증된 리스크 방지책과 예산 타격을 요구함"
        ],
        "relations": {
            "이인선": "지나치게 감정적이거나 일단 실행부터 해보자고 나설 때, 예산 낭비와 안전장치 부재를 집요하게 지적하며 제동을 걺",
            "박범서": "부정적이고 게으르게 회피하려는 무임승차적 태도를 매우 한심하게 보며 정곡을 찔러 압박함"
        }
    },
    {
        "name": "이인선", 
        "role": "실행능력이 좋은 적극적인 여성", 
        "preset": "4",
        "initiative": 0.9,
        "quirks": [
            "비즈니스 존댓말을 정중하게 사용하되, 목소리 톤에 흔들림이 없고 주장이 단호함", 
            "질질 끄는 분석보다 빠르게 구현 가능한 MVP를 만들어 바로 적용하자고 제안함"
        ],
        "relations": {
            "문경수": "계획과 걱정만 하다가 일정을 날리는 답답함에 맞서, '해보지 않고는 알 수 없는 영역이 있다'며 강경하게 맞받아침",
            "박범서": "귀찮아하고 대충 예전 거나 베끼자는 소극적인 태도에 즉각 정색하며 날카롭게 일침을 놓음"
        }
    },
    {
        "name": "배재환", 
        "role": "긍정적 소통가이며 아이디어뱅크", 
        "preset": "3",
        "initiative": 0.65,
        "quirks": [
            "상대방의 사소한 의견에도 '와, 그것 진짜 신박하고 멋진 접근이네요!' 하고 큰 리액션으로 격려함", 
            "말투가 매우 부드럽고 이모티콘이나 긍정적이고 따뜻한 단어를 적극 활용함"
        ],
        "relations": {
            "문경수": "문경수가 날카롭고 차갑게 비판하더라도 그의 철저함을 존경한다며 칭찬해 분위기가 얼어붙지 않게 함",
            "박범서": "박범서의 부정적인 한탄마저 '그만큼 신중해야 한다는 말씀이시죠!'라며 긍정적인 의미로 해석해 감싸줌"
        }
    },
    {
        "name": "박범서", 
        "role": "부정적 시각과 게으름",
        "preset": "6", 
        "custom_desc": "매사에 귀찮아하고 부정적이며 냉소적이고 적극적으로 일하지 않으려는 성향",
        "initiative": 0.25,
        "quirks": [
            "대화 도중 '굳이요?', '어차피...', '귀찮게...' 같은 무기력한 단어를 자주 내뱉음", 
            "회의가 복잡해지면 '대충 예전 거 베끼는 게 제일 안전하고 편하다'며 잔머리를 굴림",
            "한숨을 길게 쉬거나 말줄임표(...)를 자주 사용하며 열의가 없고 피곤하다는 내색을 풍김"
        ],
        "relations": {
            "이인선": "자꾸 대단한 일인 양 추진하며 나에게까지 불똥(업무 분담)을 튀기려는 기세가 매우 귀찮고 부담스러움",
            "김정준": "상냥하고 다정하게 안부를 물어올 때 귀찮지만 예의상 억지로 마지못해 짧게 대답함"
        }
    }
]
...
class Participant:
    def __init__(self, name, role, is_leader, preset_key=None, custom_desc=None, initiative=0.5, quirks=None, relations=None):
        self.name = name
        self.role = role
    ...
    def get_system_instruction(self, all_participants, meeting_state):
        member_list = "\n".join(
            f"- {p.name} ({p.role}): {p.persona_desc} (주도성: {p.initiative})" for p in all_participants
        )

        # 버릇과 관계 지침을 포맷하여 주입
        quirks_bullet = "\n".join(f"- {q}" for q in self.quirks) if self.quirks else "- 특별한 대화 버릇 없음"
        
        relations_bullet = ""
        if self.relations:
            relations_bullet = "\n".join(f"- {name}에 대한 태도: {desc}" for name, desc in self.relations.items())
        else:
            relations_bullet = "- 특이 대인 관계 주의 사항 없음"
...
[당신의 말버릇 및 시그니처 행동 패턴 (Quirks)]
{quirks_bullet}

[당신의 타 참가자에 대한 감정/관계성 (Relations)]
{relations_bullet}
"""
...
주도성 수치: {self.initiative} (가중치 기반 발언 빈도 조절)
...
{intense_debate_guideline}
[발언 지침]
1. 캐릭터 성격·말투·가치관·말버릇(Quirks)을 연극 배우처럼 극도로 유지하세요.
...
def main():
    print(Fore.CYAN + Style.BRIGHT + "==================================================")
    print(Fore.CYAN + Style.BRIGHT + "    가상 팀 회의 시뮬레이터 v4 (개성 & 주도성 도입) ")
    print(Fore.CYAN + Style.BRIGHT + "==================================================")
    ...
    for i, p_data in enumerate(SAVED_PARTICIPANTS):
        is_leader = (i == 0)
        init_val = p_data.get("initiative", 0.5)
        quirks_val = p_data.get("quirks", [])
        relations_val = p_data.get("relations", {})
        
        if p_data["preset"] == "6":
            p = Participant(
                p_data["name"], p_data["role"], is_leader=is_leader,
                custom_desc=p_data["custom_desc"],
                initiative=init_val, quirks=quirks_val, relations=relations_val
            )
        else:
            p = Participant(
                p_data["name"], p_data["role"], is_leader=is_leader,
    ...
    def get_next_speaker_fallback(current_name):
        """
        주도성(initiative) 가중치를 부여하되, 최근 발언 강도를 참작한 확률적 발언자 선택
        """
        last_spoken_idx = {p.name: -1 for p in participants}
        for idx, msg in enumerate(conversation_history):
            for p in participants:
                # 대화 내역에서 발언 여부 검출
                if msg.startswith(f"{p.name} (") or f"{p.name} (" in msg:
                    last_spoken_idx[p.name] = idx

        candidates = [p for p in participants if p.name != current_name]
        if not candidates:
            return leader.name
        ...
        weights = []
        for p in candidates:
            # 1. 기본 가중치 = 주도성 (Initiative)
            w = p.initiative

            # 2. 최근 발언도(Recency)에 따른 조정 계수 산출
            last_idx = last_spoken_idx[p.name]
            if last_idx == -1:
                # 한 번도 발언하지 않은 사람은 가장 높은 우선순위 혜택
                recency_mult = 2.5
            else:
                dist = total_turns - last_idx
                if dist <= 1:
                    recency_mult = 0.05  # 직전에 발언한 사람은 2연속 발언 억제
                elif dist == 2:
                    recency_mult = 0.35  # 2턴 전 발언자
                elif dist == 3:
                    recency_mult = 0.75  # 3턴 전 발언자
                else:
                    # 오래 발언하지 않을수록 가중치를 점진적 강화
                    recency_mult = 1.0 + (dist * 0.08)

            weights.append(max(0.01, w * recency_mult))

        # 가중치에 따라 확률적으로 발언자 선택
    ...
    for p in participants:
        prompt = (
            ...
            f"현재 기분이나 오늘 회의에 대한 기대를 당신의 개성, 말버릇(Quirks)과 성격에 맞춰 딱 1문장(짧은 구어체, 존댓말, 이모지 적극 권장)으로만 대사로 표현해 주세요.\n"
        ...
        if phase_idx == 1 and not finished_early:
            ...
            for p in participants:
                prompt = (
                    ...
                    f"당신의 고유한 말버릇(Quirks)과 성격에 맞춰 딱 1문장(짧은 구어체, 존댓말, 이모지 적극 사용)으로 나누어 주세요.\n"
...
원장에 명시된 내용만 회의록에 포함하세요."""
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"가상 팀 회의 시뮬레이터 v4 결과 보고서\n")
