...
def draw_stickman(canvas, landmarks, w, h, is_right_handed=True, racket_trail=None, hand_trail=None, phase_smoother=None, is_serve=True):
    ...
    return current_phase, racket_center
...
def track_ball_from_video(video_path, swings_info, all_racket_centers, render_w, render_h, orig_w, orig_h):
    ...
    while True:
        ...
        lower = np.array([20, 10, 80])      # Widened HSV bounds for clay court white-balance shifts
        ...
        for c in contours:
            ...
            if 0.5 < area < 400: # Widened area bounds
                M = cv2.moments(c)
                if M['m00'] > 0:
                    cx = int(M['m10'] / M['m00'])
                    cy = int(M['m01'] / M['m00'])
    ...
    ball_positions_orig = [None] * total_frames
    
    # Process each swing to track local ball segments
    for swing_idx, swing in enumerate(swings_info):
        # 1. Find the best local impact frame and position
        best_swing_f = swing[len(swing)//2]
        min_dist = float('inf')
        best_cand_pos = None
        
        for f in swing:
            r_pos = all_racket_centers[f]
            if r_pos is None:
                continue
            r_pos_orig = (r_pos[0] / scale_w, r_pos[1] / scale_h)
            
            for cx, cy, _ in all_candidates[f]:
                dist = math.sqrt((cx - r_pos_orig[0])**2 + (cy - r_pos_orig[1])**2)
                if dist < min_dist:
                    min_dist = dist
                    best_swing_f = f
                    best_cand_pos = (cx, cy)
                    
        # If candidate is close to racket, use it as impact; otherwise fall back to racket pos
        if min_dist < 45.0 and best_cand_pos is not None:
            I_k = best_swing_f
            impact_pos_orig = best_cand_pos
        else:
            I_k = swing[len(swing)//2]
            r_pos = all_racket_centers[I_k]
            if r_pos is not None:
                impact_pos_orig = (r_pos[0] / scale_w, r_pos[1] / scale_h)
            else:
                impact_pos_orig = (orig_w // 2, orig_h // 2)
                
        ball_positions_orig[I_k] = impact_pos_orig
        
        # Track backwards (Incoming ball)
        for f in range(I_k - 1, max(0, I_k - 25), -1):
            ref_pos = ball_positions_orig[f+1]
            if ref_pos is None:
                break
                
            pred_pos = ref_pos
            if f + 2 < total_frames and ball_positions_orig[f+2] is not None:
                vx = ref_pos[0] - ball_positions_orig[f+2][0]
                vy = ref_pos[1] - ball_positions_orig[f+2][1]
                pred_pos = (ref_pos[0] + vx, ref_pos[1] + vy)
                
            candidates = all_candidates[f]
            valid_cands = []
            for cx, cy, area in candidates:
                if all_racket_centers[f] is not None:
                    rx = all_racket_centers[f][0] / scale_w
                    ry = all_racket_centers[f][1] / scale_h
                    if math.sqrt((cx - rx)**2 + (cy - ry)**2) < 15:
                        continue
                valid_cands.append((cx, cy, area))
                
            if valid_cands:
                best_cand = min(valid_cands, key=lambda c: (c[0]-pred_pos[0])**2 + (c[1]-pred_pos[1])**2)
                dist_sq = (best_cand[0]-pred_pos[0])**2 + (best_cand[1]-pred_pos[1])**2
                if dist_sq < 60**2:
                    ball_positions_orig[f] = (best_cand[0], best_cand[1])
                    
        # Track forwards (Outgoing ball)
        for f in range(I_k + 1, min(total_frames, I_k + 35)):
            ref_pos = ball_positions_orig[f-1]
            if ref_pos is None:
                break
                
            pred_pos = ref_pos
            if f - 2 >= 0 and ball_positions_orig[f-2] is not None:
                vx = ref_pos[0] - ball_positions_orig[f-2][0]
                vy = ref_pos[1] - ball_positions_orig[f-2][1]
                pred_pos = (ref_pos[0] + vx, ref_pos[1] + vy)
    ...
    # Scale ball positions to render coordinates
    ball_positions = [None] * total_frames
    for f in range(total_frames):
        pos = ball_positions_orig[f]
        if pos is not None:
            ball_positions[f] = (int(pos[0] * scale_w), int(pos[1] * scale_h))
            
    # Handle single missing frames inside segments with simple interpolation
    for f in range(1, total_frames - 1):
        if ball_positions[f] is None and ball_positions[f-1] is not None and ball_positions[f+1] is not None:
            ball_positions[f] = (
                (ball_positions[f-1][0] + ball_positions[f+1][0]) // 2,
                (ball_positions[f-1][1] + ball_positions[f+1][1]) // 2
            )
            
    tracked_count = sum(1 for p in ball_positions if p is not None)
    print(f"[✓] Tracked ball from video for {tracked_count} frames.")
...
def process_video(video_url, name, is_right_handed, label=None, desc=None, speed=1.0):
    ...
    # ── FIRST PASS: Landmarker Detection ──
    print("[i] Starting First Pass: Detecting poses and analyzing motion...")
    all_smoothed = []
    all_racket_centers = []
    all_phases = []
    ...
    first_smoother = PoseSmoother(freq=fps if fps > 0 else 30.0)
    first_phase_smoother = PhaseSmoother()
    dummy_canvas = np.zeros((render_h, render_w, 3), dtype=np.uint8)
    ...
    while True:
        ...
        if result.pose_landmarks and len(result.pose_landmarks) > 0:
            ...
            smoothed = first_smoother.apply(mutable_landmarks)
            current_phase, racket_center = draw_stickman(
                dummy_canvas.copy(), smoothed, render_w, render_h, is_right_handed,
                racket_trail=None, hand_trail=None, phase_smoother=first_phase_smoother, is_serve=is_serve
            )
            
        all_smoothed.append(smoothed)
        all_racket_centers.append(racket_center)
    ...
    total_detected_frames = frame_idx
    print(f"[✓] First Pass Complete. Analyzed {total_detected_frames} frames.")

    # ── MULTI-SWING DETECTION & IMPACT POINTS ──
    impact_frames = [i for i, p in enumerate(all_phases) if p == "Impact" and all_racket_centers[i] is not None]
    
    swings = []
    if impact_frames:
        current_swing = [impact_frames[0]]
        for f in impact_frames[1:]:
            if f - current_swing[-1] <= 15:
        ...
        # Fallback to a single dummy swing in the middle
        swings = [[total_detected_frames // 2]]
        
    print(f"[i] Detected {len(swings)} swings in the video.")
    
    # ── BALL TRACKING AND INTERPOLATION ──
    ball_positions = track_ball_from_video(
        actual_input, swings, all_racket_centers, render_w, render_h, orig_w, orig_h
    )
    
    # Parabolic gap bridging and extrapolation
    valid_indices = [i for i, p in enumerate(ball_positions) if p is not None]
    if len(valid_indices) > 0:
        g = 0.15 * (render_h / 720.0)
        
        # Parabolic bridging of gaps between tracked segments
        for i in range(len(valid_indices) - 1):
            idxA = valid_indices[i]
            idxB = valid_indices[i+1]
            gap_len = idxB - idxA
            if gap_len <= 1:
                continue
            posA = ball_positions[idxA]
            posB = ball_positions[idxB]
            
            if gap_len <= 3:
                # Linear interpolation for very short gaps
                for t in range(idxA + 1, idxB):
                    frac = (t - idxA) / gap_len
                    x = int(posA[0] * (1 - frac) + posB[0] * frac)
                    y = int(posA[1] * (1 - frac) + posB[1] * frac)
                    ball_positions[t] = (x, y)
            else:
                # Parabolic interpolation
                vx = (posB[0] - posA[0]) / gap_len
                vy = (posB[1] - posA[1] - 0.5 * g * gap_len**2) / gap_len
                for t in range(idxA + 1, idxB):
                    dt = t - idxA
                    x = int(posA[0] + vx * dt)
                    y = int(posA[1] + vy * dt + 0.5 * g * dt**2)
                    ball_positions[t] = (x, y)
                    
        # Backward extrapolation from first valid index
        first_valid = valid_indices[0]
        if first_valid > 0:
            ref_idx = min(total_detected_frames - 1, first_valid + 2)
            if ref_idx > first_valid:
                dt = ref_idx - first_valid
                vx = (ball_positions[ref_idx][0] - ball_positions[first_valid][0]) / dt
                vy = (ball_positions[ref_idx][1] - ball_positions[first_valid][1] - 0.5 * g * dt**2) / dt
                for t in range(first_valid - 1, -1, -1):
                    dt_ext = t - first_valid
                    x = int(ball_positions[first_valid][0] + vx * dt_ext)
                    y = int(ball_positions[first_valid][1] + vy * dt_ext + 0.5 * g * dt_ext**2)
                    if 0 <= x < render_w and 0 <= y < render_h:
                        ball_positions[t] = (x, y)
                    else:
                        break
                        
        # Forward extrapolation from last valid index
        last_valid = valid_indices[-1]
        if last_valid < total_detected_frames - 1:
            ref_idx = max(0, last_valid - 2)
            if last_valid > ref_idx:
                dt = last_valid - ref_idx
                vx = (ball_positions[last_valid][0] - ball_positions[ref_idx][0]) / dt
                vy = (ball_positions[last_valid][1] - ball_positions[ref_idx][1] - 0.5 * g * dt**2) / dt
                for t in range(last_valid + 1, total_detected_frames):
                    dt_ext = t - last_valid
                    x = int(ball_positions[last_valid][0] + vx * dt_ext)
                    y = int(ball_positions[last_valid][1] + vy * dt_ext + 0.5 * g * dt_ext**2)
                    if 0 <= x < render_w and 0 <= y < render_h:
                        ball_positions[t] = (x, y)
                    else:
                        break
                        
    # Find local impact frames and positions to display shockwaves
    impact_points = []
    for swing in swings:
        best_swing_f = swing[len(swing)//2]
        min_dist = float('inf')
        for f in swing:
            if all_racket_centers[f] is not None and ball_positions[f] is not None:
                rx, ry = all_racket_centers[f]
        ...
        imp_pos = all_racket_centers[best_swing_f]
        if imp_pos is None:
            imp_pos = ball_positions[best_swing_f]
        if imp_pos is None:
            imp_pos = (render_w // 2, render_h // 2)
            
        impact_points.append((best_swing_f, imp_pos))
        print(f"[i] Impact Frame identified at {best_swing_f}, position: {imp_pos}")

    # ── SECOND PASS: Rendering ──
    print("[i] Starting Second Pass: Rendering stickman and ball effects...")
    cap = cv2.VideoCapture(actual_input)
    frame_idx = 0
    
    second_phase_smoother = PhaseSmoother()
    racket_trail = []
    hand_trail = []
    ball_trail = []
    ...
    while True:
        ...
        smoothed = all_smoothed[frame_idx]
        if smoothed is not None:
            current_phase, _ = draw_stickman(
                canvas, smoothed, render_w, render_h, is_right_handed,
                racket_trail, hand_trail, second_phase_smoother, is_serve=is_serve
            )
        ...
        if ball_pos is not None:
            ...
            if len(ball_trail) >= 2:
                ...
                for i in range(1, len(ball_trail)):
                    t = i / (len(ball_trail) - 1)
                    thick = max(1, int(10 * LW * t))
                    cv2.line(overlay, ball_trail[i-1], ball_trail[i], (0, 245, 255), thick, cv2.LINE_AA)
                cv2.addWeighted(overlay, 0.35, canvas, 0.65, 0, canvas)
                ...
                for i in range(1, len(ball_trail)):
                    t = i / (len(ball_trail) - 1)
                    thick = max(1, int(3 * LW * t))
                    cv2.line(overlay_core, ball_trail[i-1], ball_trail[i], (255, 255, 255), thick, cv2.LINE_AA)
                cv2.addWeighted(overlay_core, 0.65, canvas, 0.35, 0, canvas)
                
            ball_r = max(int(6 * LW), 5)
            cv2.circle(canvas, ball_pos, ball_r + 2, (0, 230, 255), -1, cv2.LINE_AA)
            cv2.circle(canvas, ball_pos, ball_r, (255, 255, 255), -1, cv2.LINE_AA)
            cv2.ellipse(canvas, (ball_pos[0] - int(ball_r * 0.75), ball_pos[1]), 
                        (int(ball_r * 0.75), int(ball_r * 0.75)), 0, -45, 45, (0, 180, 200), 1, cv2.LINE_AA)
            cv2.ellipse(canvas, (ball_pos[0] + int(ball_r * 0.75), ball_pos[1]), 
                        (int(ball_r * 0.75), int(ball_r * 0.75)), 0, 135, 225, (0, 180, 200), 1, cv2.LINE_AA)
            
        # Draw Impact Shockwave and Text for all impacts
        for imp_idx, imp_pos in impact_points:
            if 0 <= frame_idx - imp_idx < 8:
                k = frame_idx - imp_idx
                sh_r = int(max(int(10 * LW), 10) * (k + 1) * 1.5)
                alpha = max(0.1, 1.0 - k * 0.12)
                
                overlay = canvas.copy()
                cv2.circle(overlay, imp_pos, sh_r, (100, 255, 255), max(1, int(6 * LW) - k), cv2.LINE_AA)
                cv2.circle(overlay, imp_pos, int(sh_r * 0.7), (255, 255, 255), max(1, int(3 * LW) - k), cv2.LINE_AA)
                cv2.addWeighted(overlay, alpha * 0.5, canvas, 1.0 - alpha * 0.5, 0, canvas)
                
                font = cv2.FONT_HERSHEY_SIMPLEX
                font_scale = 1.6 * LW
                text_x = imp_pos[0] - int(55 * LW)
                text_y = imp_pos[1] - int(35 * LW)
                cv2.putText(canvas, "POW!", (text_x, text_y), font, font_scale, (20, 20, 20), int(6 * LW), cv2.LINE_AA)
                cv2.putText(canvas, "POW!", (text_x, text_y), font, font_scale, (0, 245, 255), int(2 * LW), cv2.LINE_AA)
                break
        ...
        frame_idx += 1
        if frame_idx % 30 == 0:
            pct = frame_idx / total_detected_frames * 100 if total_detected_frames > 0 else 0
            print(f"  Rendering: frame {frame_idx}/{total_detected_frames} ({pct:.1f}%)")
            
    cap.release()
    out.write(final)
    out.release()
    print(f"[✓] Stickman video saved: {output_path}")
...
if __name__ == "__main__":
    args = parse_args()
    print("=" * 60)
    print(f"  TENNIS STICKMAN ANIMATION GENERATOR v6")
