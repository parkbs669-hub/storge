...
# -*- coding: utf-8 -*-
"""
generate_tennis_backhand.py - 테니스 백핸드 스윙 졸라맨 애니메이션 생성기
- 카를로스 알카라스 실제 백핸드 뒷 모습 영상 모션 캡처 데이터 적용 (MediaPipe Pose 기반)
- 화이트보드 마커 드로잉 스타일 적용 (흰 배경에 검은 잉크)
- 2D IK(Inverse Kinematics) 솔버를 통한 뼈대 강체 길이 유지 및 오클루전 보정
- Gaussian 시간 축 평활화(Temporal Smoothing)로 프레임 간 떨림(jitter) 최소화
...
FPS = 30
TOTAL_FRAMES = 180
OUTPUT_PATH = "tennis_backhand_pro.mp4"

# --- 모션 캡처 데이터 로드 ---
POSE_DATA_PATH = r"c:\Users\bagch\Downloads\text_to_video_project\user_videos\pose_data_backview.json"
...
# --- 2D IK 솔버 (관절 길이 유지 및 자연스러운 굽힘 계산) ---
def solve_ik_joint(start, end, raw_mid, l1, l2):
    dx = end[0] - start[0]
    dy = end[1] - start[1]
    ...
    if d >= l1 + l2:
        # 완전 직선으로 펴진 경우
        if d == 0:
            return (start[0], start[1] + l1)
        return (start[0] + (l1 / d) * dx, start[1] + (l1 / d) * dy)
    elif d <= abs(l1 - l2):
        # 겹치거나 과도하게 꺾인 경우, 일직선 상에 배치
        if d == 0:
            return (start[0], start[1] + l1)
        return (start[0] + (l1 / d) * dx, start[1] + (l1 / d) * dy)
    else:
        ...
        # 교점은 2개 존재
        e1 = (px + (h / d) * dy, py - (h / d) * dx)
        e2 = (px - (h / d) * dy, py + (h / d) * dx)
        
        # 실제 영상의 관절(raw_mid)과 더 가까운 쪽을 선택
        d1 = (e1[0] - raw_mid[0])**2 + (e1[1] - raw_mid[1])**2
        d2 = (e2[0] - raw_mid[0])**2 + (e2[1] - raw_mid[1])**2
...
# --- 가우시안 필터 적용 스무딩 함수 ---
def get_smoothed_landmarks_at_time(t_video, window=7):
    lms_list = []
    weights = []
    half = window // 2
    for offset in range(-half, half + 1):
        t_check = t_video + offset
        t_check = max(0.0, min(len(POSE_FRAMES) - 1, t_check))
        lms = get_landmarks_at_time(t_check)
        if lms:
            # Gaussian weight (sigma = 1.5)
            w = math.exp(-(offset**2) / (2.0 * (1.5**2)))
            lms_list.append(lms)
            weights.append(w)
    ...
    smoothed = {}
    ref_lms = lms_list[0]
    for key in ref_lms.keys():
        smoothed[key] = {"x": 0.0, "y": 0.0, "z": 0.0}
        w_sum = 0.0
        for lms, w in zip(lms_list, weights):
            if key in lms:
                smoothed[key]["x"] += lms[key]["x"] * w
                smoothed[key]["y"] += lms[key]["y"] * w
                smoothed[key]["z"] += lms[key]["z"] * w
                w_sum += w
        if w_sum > 0:
            smoothed[key]["x"] /= w_sum
            smoothed[key]["y"] /= w_sum
            smoothed[key]["z"] /= w_sum
        else:
            smoothed[key] = ref_lms[key]
            
    return smoothed

# --- 모션 캡처 기반 졸라맨 좌표 계산 함수 ---
def get_captured_joints(frame_idx, stickman_x, stickman_y, char_scale):
    # 애니메이션 프레임(0-180)을 비디오 프레임(48-282, 알카라스 백뷰 스윙 구간)으로 매핑
    # 임팩트를 프레임 100에 맞추기 위해 구간별 선형 매핑 적용
    # - 0 ~ 100 프레임: 비디오 48 ~ 160 프레임 (준비, 백스윙 및 다운스윙)
    # - 100 ~ 180 프레임: 비디오 160 ~ 282 프레임 (팔로우 스루 및 피니시/복귀)
    if frame_idx <= 100:
        t_video = 48.0 + frame_idx * (160.0 - 48.0) / 100.0
    else:
        t_video = 160.0 + (frame_idx - 100.0) * (282.0 - 160.0) / 80.0
        
    lms = get_smoothed_landmarks_at_time(t_video, window=7)
    if not lms:
        return None
    ...
    # 2D 좌표 리타게팅 함수 (힙 센터 기준 상대 좌표)
    def map_joint(v_pos):
        rel = v_pos - hip_center
        return (stickman_x + rel[0] * scale, stickman_y + rel[1] * scale)
        
    neck_mapped = map_joint(neck)
    head_mapped = map_joint(nose)
    
    raw_l_elbow = map_joint(l_el)
    raw_r_elbow = map_joint(r_el)
    l_wrist_mapped = map_joint(l_wr)
    r_wrist_mapped = map_joint(r_wr)
    
    raw_l_knee = map_joint(l_kn)
    raw_r_knee = map_joint(r_kn)
    l_ankle_mapped = map_joint(l_ak)
    r_ankle_mapped = map_joint(r_ak)
    
    # 뼈대 강체성 유지를 위한 IK 솔버 적용
    l1_arm = 60.0 * char_scale
    l2_arm = 60.0 * char_scale
    l1_leg = 70.0 * char_scale
    l2_leg = 70.0 * char_scale
    
    l_elbow_solved = solve_ik_joint(neck_mapped, l_wrist_mapped, raw_l_elbow, l1_arm, l2_arm)
    r_elbow_solved = solve_ik_joint(neck_mapped, r_wrist_mapped, raw_r_elbow, l1_arm, l2_arm)
    
    l_knee_solved = solve_ik_joint((stickman_x, stickman_y), l_ankle_mapped, raw_l_knee, l1_leg, l2_leg)
    r_knee_solved = solve_ik_joint((stickman_x, stickman_y), r_ankle_mapped, raw_r_knee, l1_leg, l2_leg)
    
    joints = {
        "hip": (stickman_x, stickman_y),
        "neck": neck_mapped,
        "head": head_mapped,
        "l_elbow": l_elbow_solved,
        "l_wrist": l_wrist_mapped,
        "r_elbow": r_elbow_solved,
        "r_wrist": r_wrist_mapped,
        "l_knee": l_knee_solved,
        "l_ankle": l_ankle_mapped,
        "r_knee": r_knee_solved,
        "r_ankle": r_ankle_mapped,
        "head_r": 25.0 * char_scale,
        "racket_len": 70.0 * char_scale
    }
    
    # 라켓 각도 계산 (손목 -> 검지 방향 벡터 활용 및 노이즈 예외 처리)
    racket_dx = r_idx[0] - r_wr[0]
    racket_dy = r_wr[1] - r_idx[1]
    dist = math.sqrt(racket_dx**2 + racket_dy**2)
    if dist < 0.005:
        # 손목과 검지가 겹친 경우, 오른쪽 팔꿈치 -> 손목 방향으로 보완
        r_dx = r_wr[0] - r_el[0]
        r_dy = r_el[1] - r_wr[1]
        dist2 = math.sqrt(r_dx**2 + r_dy**2)
        if dist2 < 0.005:
            racket_rad = math.radians(45)
        else:
            racket_rad = math.atan2(r_dy, r_dx)
    else:
        racket_rad = math.atan2(racket_dy, racket_dx)
        
    joints["racket_rad"] = racket_rad
...
def build_tennis_video():
    print(f"[Start] Alcaraz Back-View Backhand Animation rendering started (Resolution: {WIDTH}x{HEIGHT}, FPS: {FPS})")
    ...
    for f_idx, state in enumerate(simulated_positions):
        ...
        tips = {
            "1. 준비 자세 (Ready)": "몸을 정면으로 향하고 양손으로 라켓을 가볍게 잡습니다.",
            "2. 백스윙 (Backswing)": "어깨를 왼쪽으로 깊게 돌려 라켓을 왼쪽 뒤로 당깁니다.",
            "3. 라켓 드롭 (Racket Drop)": "라켓 헤드를 아래로 떨어뜨려 상향 스윙을 준비합니다.",
            "4. 임팩트 (Impact)": "오른발을 딛으며 양손으로 볼을 강하게 때립니다.",
            "5. 팔로우 스루 (Follow Through)": "라켓을 오른쪽 어깨 위로 깊숙이 들어 올려 마무리합니다.",
            "6. 피니시 (Finish)": "동작을 끝내고 다음 랠리를 위해 준비 자세로 돌아옵니다."
        }
